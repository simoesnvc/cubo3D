...
static int	check_walls(t_game *game)
{
	...
	i = 0;
	y = 0;
	while (game->map[0][y] != '\n' && game->map[0][y] != 0) //verifica a linha i = 0
	{
		if ((game->map[0][y] != '1') && (game->map[0][y] != ' '))
			return (-1);
		y++;
	}
	while (game->map[i] != 0 && game->map[i][0] != '\n' && game->map[i][0] != 0) //verifica a coluna y = 0
	{
	...
	y = 0;
	i--;
	while (game->map[i][y] != '\n' && game->map[i][y] != 0) //verifica ultima linha i = max_y
	{
	...
	i--;
	while (i >= 0) //verifica coluna da direita y = strlen
	{
		if (game->map[i][ft_strlen(game->map[i]) - 2] != '1' && game->map[i][ft_strlen(game->map[i]) - 2] != ' ')
			return (-1);
		i--;
	}
	return (0);
...
static int	valid_space(t_game *game, int i, int j)
{
	...
	if (j <= 0 || !valid_adjacent_char(game->map[i][j - 1]))
		return (-1);
	if (j + 1 >= (int)ft_strlen(game->map[i]) || !valid_adjacent_char(game->map[i][j + 1]))
		return (-1);
	return (0);
}
...
int	validate_map(t_game *game)
{
	...
	while (game->map[i] != 0)
	{
		...
		while (game->map[i][j] != 0 && game->map[i][j] != '\n')
		{
			...
			{
				if (valid_space(game, i, j) == -1 || save_position(game, i, j) == -1)
					return (-1);
